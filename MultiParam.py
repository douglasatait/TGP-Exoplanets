...
def magplot(dataf,datam):
	...
	plt.errorbar(t,MAG_APER, MAGERR_APER)
	plt.xlabel("time since transit [days]")
	plt.ylabel("Magnitude")
	plt.title('WASP Mag')
	plt.show()

	plt.errorbar(t,FLUX_APER, FLUXERR_APER)
	plt.xlabel("time since transit [days]")
	plt.ylabel("Flux")
	plt.title("Wasp Flux")
	...
	plt.plot(t,MAG_APER)
	plt.plot(t,MAG_APER_C1)
	plt.plot(t,MAG_APER_C2)
	plt.plot(t,MAG_APER_C3)
...
def AtEx(data):
	...
	'''
	plt.errorbar(t,FLUX_APER_C1,yerr=FLUXERR_APER_C1,ecolor='k', label='$C_1$',color='red')
	plt.plot(t,FLUX_APER_C2, label='$C_2$',color='blue')
	plt.plot(t,FLUX_APER_C3, label='$C_3$',color='k')
	plt.hlines(C1_mean_FLUX,min(t),max(t),color='red')
	plt.hlines(C2_mean_FLUX,min(t),max(t), color='blue')
	plt.hlines(C3_mean_FLUX,min(t),max(t),color='k')
	#plt.plot(t, AVG_FLUX_FLUCT)
	plt.title("Mean fluxes of comparison stars")
	plt.xlabel("time since transit [days]")
	plt.ylabel("Flux (counts s$^{-1}$)")
	plt.legend()
	plt.show()
	...
	plt.errorbar(t,C1_Fluct_FLUX,yerr=c1err,ecolor='k', label='$C_1$')
	plt.plot(t,C2_Fluct_FLUX, label='$C_2$')
	plt.plot(t,C3_Fluct_FLUX, label='$C_3$')
	#plt.plot(t, AVG_FLUX_FLUCT)
	plt.title("Ratio of flux extinction from comparison stars")
	plt.xlabel("time since transit [days]")
	...
	plt.plot(t,Flux_C1,label='$C_1$ corr')
	plt.plot(t,Flux_C2,label='$C_2$ corr')
	plt.plot(t,Flux_C3,label='$C_3$ corr')
	...
	#plt.axvline(0.0377,color='y')#end transit
	plt.title("Comparison Star Corrected Fluxes")
	plt.xlabel("time since transit [days]")
	plt.ylabel("Flux (counts s$^{-1}$)")
	plt.legend()
	plt.show()
	...
	#plt.axvline(-0.0297,color='k')  #45 sec exposure
	#plt.axvline(0.0,color='k')  #30 sec exposure
	plt.errorbar(t,Flux_WASP,yerr=fwerr,ecolor='r',label='corrected')
	plt.errorbar(t,FLUX_APER,yerr=FLUXERR_APER,ecolor='k',label='observed')
...
def batmanCurveFitee0(xdata,rp0,inc0,w0):
    params = batman.TransitParams()
...
def batmanCurveFite1(xdata,rp0,inc0,w0):
    params = batman.TransitParams()
...
def batmanCurveFite2(xdata,rp0,inc0,w0):
    params = batman.TransitParams()
...
def batmanCurveFite3(xdata,rp0,inc0,w0):
    params = batman.TransitParams()
...
def batmanCurveFitTmax(xdata,rp0,inc0,e0,w0):	# Tmax , a_mean
    params = batman.TransitParams()
    ...
    params.inc = inc0                      #orbital inclination (in degrees)
    params.ecc = 0.069                       #eccentricity
    params.w =  e0                       #longitude of periastron (in degrees)
    params.limb_dark = "quadratic"        #limb darkening model
    params.u = [0.525, 0.188]      #limb darkening coefficients [u1, u2]
...
def batmanCurveFite0(xdata,rp0,inc0,w0):
    params = batman.TransitParams()
...
def LDeModels(t,data,err,ig):
	...
	##### uniform fit, linear fit, quadratic fit, nonlinear fit #####
		fitThis=spicy.curve_fit(f = funcs[i], xdata = t, ydata = data, sigma=s_err, p0=ig, bounds=(0,[1.,90.,360.]))
		abest, bbest, cbest = fitThis[0]
		aerr = np.sqrt(fitThis[1][0][0])  
		berr = np.sqrt(fitThis[1][1][1])
		cerr = np.sqrt(fitThis[1][2][2])      
		flux=funcs[i](t,abest,bbest,cbest)								
...
def findParamse0(t,W_Flux,nWerr,ig,func):
	...
	s_err=scaleError(nWerr,4.5)
	fitThis = spicy.curve_fit(f = func, xdata = t, ydata = W_Flux, sigma=s_err, p0=ig, bounds=(0,[1.,90.,360.]))
	abest,bbest,cbest = fitThis[0]
...
def plotLCe0(t,data,err,params):
	# plot light curve for 4 best fit parameters for Rp, incl, ecc, LoP
	abest,bbest,cbest=params
	flux= batmanCurveFite0(t,abest,bbest,cbest)
	s_err=scaleError(err,4.5)					# scales errors by factor of 1/4.5
...
def modelcomp(t,data,err,m1,m2):
	...
	rchs2=chsq(data,m2,s_err,[1,2,3])
...
def main():
	...
	m2=batmanCurveFite0(t,paramse0[0],paramse0[1],paramse0[2])							# returns model for fixed eccentricity
